...
const getServiceIcon = (serviceName) => {
    ...
    if (s.includes('elec') || s.includes('fan') || s.includes('switch') || s.includes('power')) {
        return { icon: Zap, bg: 'bg-amber-50 text-amber-600', border: 'border-amber-200' };
    }
    if (s.includes('ac') || s.includes('cool') || s.includes('air')) {
        return { icon: Wind, bg: 'bg-cyan-50 text-cyan-600', border: 'border-cyan-200' };
    }
    if (s.includes('plumb') || s.includes('pipe') || s.includes('leak') || s.includes('tap')) {
        return { icon: Wrench, bg: 'bg-blue-50 text-blue-600', border: 'border-blue-200' };
    }
    if (s.includes('carp') || s.includes('wood') || s.includes('door') || s.includes('furniture')) {
        return { icon: Hammer, bg: 'bg-orange-50 text-orange-600', border: 'border-orange-200' };
    }
    if (s.includes('paint') || s.includes('wall')) {
        return { icon: Paintbrush, bg: 'bg-purple-50 text-purple-600', border: 'border-purple-200' };
    }
    if (s.includes('clean') || s.includes('sanitize') || s.includes('pest')) {
        return { icon: Sparkles, bg: 'bg-emerald-50 text-emerald-600', border: 'border-emerald-200' };
    }
    return { icon: Wrench, bg: 'bg-orange-50 text-orange-600', border: 'border-orange-200' };
};
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-surface p-6 flex flex-col space-y-4 max-w-4xl mx-auto pt-16">
                <div className="h-9 bg-gray-200 rounded-xl w-1/3 mb-6 animate-pulse"></div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
                    {[1, 2, 3].map(i => <div key={i} className="h-24 bg-gray-200 rounded-2xl animate-pulse"></div>)}
                </div>
                {[1, 2, 3].map(i => <div key={i} className="h-40 bg-gray-200 rounded-3xl w-full animate-pulse"></div>)}
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-surface pb-24 pt-6 px-4 sm:px-6">
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-navy-100 pb-5">
                    <div className="flex items-center gap-3">
                        <button 
                            ...
                            className="p-2.5 rounded-xl bg-white border border-navy-100 text-navy-600 hover:text-orange-600 hover:border-orange-300 transition-all shadow-xs"
                            title="Back to Home"
                        >
                            <ArrowLeft size={18} />
                        </button>
                        <div>
                            <div className="flex items-center gap-2.5 flex-wrap">
                                <h1 className="font-extrabold text-navy-900 text-2xl tracking-tight">
                                    {t('Service History')}
                                </h1>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                    {/* Total Services */}
                    <div className="bg-white border border-navy-100 rounded-2xl p-4 shadow-xs flex items-center gap-3.5">
                        ...
                        <div>
                            <div className="text-2xl font-black text-navy-900 font-mono leading-none">
                                {history.length}
                            </div>
                            <div className="text-xs font-bold text-navy-700 mt-1">
                                Total Bookings
                            </div>
                    ...
                    <div className="bg-white border border-navy-100 rounded-2xl p-4 shadow-xs flex items-center gap-3.5">
                        ...
                        <div>
                            ...
                            <div className="text-xs font-bold text-navy-700 mt-0.5">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-1">
                    {/* Tabs */}
                    <div className="inline-flex p-1 bg-navy-50 rounded-xl border border-navy-100 self-start">
                        <button
                    ...
                    <div className="relative w-full sm:w-64">
                        ...
                        <input
                            ...
                            className="w-full pl-9 pr-3.5 py-1.5 bg-white text-xs font-medium text-navy-900 placeholder-navy-400 border border-navy-100 rounded-xl focus:outline-none focus:border-orange-500 shadow-2xs"
                        />
                    </div>
                </div>
                ...
                {filteredHistory.length === 0 ? (
                    <div className="text-center py-16 bg-white rounded-3xl border border-navy-100 shadow-xs mt-2">
                        <div className="w-16 h-16 bg-navy-50 rounded-2xl flex items-center justify-center mx-auto mb-3.5">
                        ...
                        {searchQuery && (
                            <button
                                ...
                                className="mt-3 px-3.5 py-1.5 bg-navy-100 text-navy-700 text-xs font-bold rounded-lg hover:bg-navy-200 transition-colors"
                            >
                                Clear Search
                            </button>
                        )}
                    ...
                    <div className="space-y-3.5">
                        {filteredHistory.map(req => {
                            ...
                            // Formatted Order ID
                            const rawCode = req.booking_code || 
                                            req.receipt_number || 
                            ...
                            const orderCode = String(rawCode).startsWith('REQ-') || String(rawCode).startsWith('ORD-')
                                ? rawCode
                                : `REQ-${String(rawCode).slice(0, 6).toUpperCase()}`;
                            ...
                            return (
                                <div 
                                    ...
                                    className="bg-white border border-navy-100 rounded-2xl p-4 sm:p-5 shadow-[0_2px_10px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] hover:border-navy-200 transition-all cursor-pointer relative group"
                                >
                                    ...
                                    <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
                                        <div className="flex items-center gap-2.5 flex-wrap">
                                            ...
                                            {/* Service ID Badge */}
                                            <span className="text-xs font-mono font-semibold text-navy-600 bg-navy-50 px-2.5 py-1 rounded-lg border border-navy-100">
                                                {serviceCode}
                                            </span>
                                        </div>
                                    ...
                                    <div className="mb-3">
                                        <h3 className="font-extrabold text-navy-900 text-base sm:text-lg group-hover:text-orange-600 transition-colors">
                                            {serviceName}
                                        </h3>
                                    ...
                                    <div className="flex flex-wrap items-center gap-y-2 gap-x-4 text-xs text-navy-500 py-2.5 border-y border-navy-100">
                                        {/* Date */}
                                        <div className="flex items-center gap-1.5">
                                            <Clock size={13} className="text-navy-400" />
                                            <span>
                                                {isCompleted ? 'Completed on' : 'Cancelled on'} <strong className="text-navy-700">{formattedDate}</strong>
                                            </span>
                                        </div>
                                        ...
                                        {req.pillar_id && (
                                            <div className="flex items-center gap-1.5">
                                                <UserCheck size={13} className="text-navy-400" />
                                                <span>Pillar: <strong className="text-navy-700">{pillarName}</strong> ({pillarRating})</span>
                                            </div>
                                        )}

                                        {/* Cancellation Reason if cancelled */}
                                        {!isCompleted && req.cancel_reason && (
                                            <div className="text-rose-600 bg-rose-50 px-2 py-0.5 rounded text-[11px] font-medium border border-rose-200">
                                    ...
                                    <div className="flex items-center justify-between gap-3 mt-3.5 pt-1 flex-wrap">
                                        ...
                                        <div className="flex items-baseline gap-2">
                                            {isCompleted ? (
                                                <>
                                                    <span className="text-xs text-navy-400 font-medium">Total Paid:</span>
                                                    <span className="text-base font-black text-navy-900 font-mono">
                                                ...
                                                <>
                                                    <span className="text-xs text-navy-400 font-medium">Charge:</span>
                                            ...
                                            {isCompleted ? (
                                                <>
                                                    <button
                                                        ...
                                                        className="px-3 py-1.5 rounded-xl border border-navy-200 text-navy-700 hover:bg-navy-50 text-xs font-bold transition-all"
                                                    >
                                                        Details
                                                    </button>
                                                    <button
